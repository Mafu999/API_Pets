    ...
    verDogs(){
        return new Promise((resolve, reject)=>{
            ...
            this.bd.all(query, (error, response)=>{
                if(error) reject(`Erro ao cessar o banco de dados. ${error}`)
    ...
    NewDog(infos){
        return new Promise((resolve, reject)=>{
            ...
            this.bd.run(query, parametros, (error, response)=>{
                if(error) reject(`Erro ao adicionar o doguinho. ${error}`)
    ...
    DeleteDogs(id){
        return new Promise((resolve, reject)=>{
            ...
            this.bd.run(query, id, (error, response)=>{
                if(error) reject(`Erro ao matar o dog. ${error}`)
                else resolve('The dog is dead')
            })
        })
    }
